...
test(`Update the petOwner via HTTP PUT`, async () => {
...
test(`Get the updated petOwner via HTTP GET`, async () => {
...
test('Update the pet via HTTP PUT', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.data.name).toBe(newName);
    console.log(`Pet Owner with id ${pet.id} name changed to ${newName}`);
    pet = response.data;
});
...
// Clean Up
test('Delete the petOwner via HTTP DELETE', async () => {
    // nothing to do yet
